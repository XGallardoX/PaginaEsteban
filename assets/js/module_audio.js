...
// Ajusta estos nombres al orden real de tu modelo de audio
...
export class AudioModule {
  constructor() {
    ...
    this.about = `
      ...
      <p>Modelo de audio basado en GTZAN convertido a TensorFlow.js.</p>
      <p>Para simplificar, toma clips cortos desde el micrófono y los clasifica en 5 géneros.</p>
      <p>Puedes cambiar los nombres de los géneros editando <code>GENRE_LABELS</code> en <code>module_audio.js</code>.</p>
    `;
  }
  ...
  async _ensureModel() {
    ...
    try {
      ...
      setStatus('Modelo de audio listo. Usa "Grabar" o sube un clip.', 'ok');
    } catch (err) {
      console.error(err);
  ...
  async _runInference(audioBuffer) {
    ...
    if (this.confBar) {
      // la barra original va de 0 a 1
      this.confBar.value = top1.prob || 0;
    }
    if (this.confVal) {
      this.confVal.textContent = `${Math.round((top1.prob || 0) * 100)}%`;
    }
    if (this.bpmEl) {
      this.bpmEl.textContent = 'BPM: —';
    }
    if (this.stabilityEl) {
      this.stabilityEl.textContent = 'Ritmo: —';
    }

    setStatus(`Audio: predicción "${top1.label}"`, 'ok');
  }
}
